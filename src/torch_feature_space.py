...
def analyse_net(net, quiet=True):
    module_dict = {}
    module_count = 0
...
def filter_dnn_feature_maps(data, _fmaps_fn, fmap_max=1024, concatenate=True, trn_size=None):
...
#    _fmaps = _fmaps_fn(_to_torch(data[:batch_size], device=device))
    fmaps_fn = lambda x: [np.copy(_fm.data.cpu().numpy()) for _fm in _fmaps_fn(x.to(device))]
    fr,fl = next(iter(data))
    fmaps = fmaps_fn(fr.to(device))
