...
def iterate_subject_ordering_minibatches(inputs, targets, image_ordering, batchsize, shuffle=False):
    ...
    if shuffle:
        ...
        islices = np.array(islices, dtype=object)[ordering]
        vslices = np.array(vslices, dtype=object)[ordering]
    else:
        subs   = np.array(subs)#, dtype=int)
        islices = np.array(islices, dtype=object)#, dtype=int)
        vslices = np.array(vslices, dtype=object)#, dtype=int)
